...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
    ...
    float predictedRoll = phi + dtIMU * euler_dot.x;
    float predictedPitch = theta + dtIMU * euler_dot.y;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
    ...
    V3F accI = attitude.Rotate_BtoI(accel) + V3F(0.f, 0.f, -9.81);
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
    ...
    if (z(0) - zFromX(0) > M_PI) z(0) -= 2.f * M_PI;
    else if (z(0) - zFromX(0) < -M_PI) z(0) += 2.f * M_PI;
